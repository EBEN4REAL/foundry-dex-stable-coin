...
contract DSCEngine is ReentrancyGuard {
    ...
    */
    ...
    function _calculateHealthFactor(uint256 totalDscMinted, uint256 collateralValueInUsd)
        ...
        returns (uint256)
    {
        // 1. Solidity can’t divide by zero. If the user has no debt, their position is perfectly safe, so return max uint.
        if (totalDscMinted == 0) return type(uint256).max;
        // Multiplying by LIQUIDATION_THRESHOLD (50) knocks it down to 50 % of its value.
